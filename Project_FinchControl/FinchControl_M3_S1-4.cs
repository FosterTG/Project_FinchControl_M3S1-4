...
namespace Project_FinchControl
{
    ...
    class Program
    {
        ...
        // option d
        static double LightAlarDisplaySetTimeToMonitor()
        {
            ...
            do
            {
                Console.WriteLine($"\tSet time to monitor in seconds:");

                userResponse = Console.ReadLine();
                if (!double.TryParse(userResponse, out timeToMonitor))
                {
                    Console.WriteLine("Please Enter A valid Response");
                    validResponse = false;
                }
            ...
            Console.WriteLine();
            Console.WriteLine("\tTime to monitor is set to " + timeToMonitor + " seconds");

            DisplayMenuPrompt("Light Alarm");
            return timeToMonitor;
        }
        //option e
        static void LightAlarmSetAlarm(Finch finchRobot,string sensersToMonitor,string rangeType,int minMaxThresholdValue,double timeToMonitor)
            ...
            while ((secondselapised < timeToMonitor) && !thresholdexited)
            {
                switch (sensersToMonitor)
                {
                    case "left":
                        ...
                        Console.WriteLine($"Left light sensor value is: {currentSensorValue}");
                        break;
                    case "right":
                        currentSensorValue = finchRobot.getRightLightSensor();
                        Console.WriteLine($"Right light sensor value is: {currentSensorValue}");
                        break;
                    case "both":
                        currentSensorValue = (finchRobot.getLeftLightSensor() + finchRobot.getRightLightSensor()) / 2;
                        Console.WriteLine($"Both light sensor value's are: {currentSensorValue}");
                        break;
                }
        ...
        static int LightAlarmDisplaySetMinMaxThresholdValue(Finch finchRobot, string rangeType)
        {
            ...
            do
            {
                ...
                if (!int.TryParse(Console.ReadLine(), out minMaxThresholdValue) || minMaxThresholdValue < 0 || minMaxThresholdValue > 255)
                {
                    Console.WriteLine("Please Enter A valid Response");
                    validResponse = false;
                }
        ...
        static string LightAlarmDisplaySetSensorsToMonitor()
        {
            ...
            // Getting Sensor to monitor and validating 
            do
            {
            ...
            Console.WriteLine();
            Console.WriteLine("\t" + sensorsToMonitor + " snesor/s is set to monitor");
        ...
        static string LightAlarmDisplaySetRangeType()
        {
            ...
            // Getting Range Type and Validating
            do
            {
            ...
            Console.WriteLine();
            Console.WriteLine("\tRange type is set to "+ rangeType);
